...
%% ---------------------------------------------------------------------
 ...
 \section{ Result }
	...
	\subsection{Rough Dimensionality descriptor}
		...
		\myimage{"./illustrations/chap2/comparing_dim_desc/explaining_failure_case"}{Representative patches for $\lvert Dim_{LOD}$-$Dim_{cov}>0.5\rvert$. Most differences are explained by $Dim_{cov}$ limitations (See \ref{lod.result.dim_failure}.)
		}{lod.fig:explaining_failure_case}
	...
	\subsection{Patch Classification}
		...
		\myimage{./illustrations/chap2/classif/result_vosges/result_vosges_2}{Vosges dataset. (table 2) Precision(prec.), recall (rec.), support (supp.), and average percent of points of the class in the patches, for comparison with point based method (mix.). (table 1)Feature usage }{lod.fig:result-vosges}
