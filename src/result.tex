...
%% ---------------------------------------------------------------------
 ...
 \section{ Result }
	...
	%\subsection{Data set used}
		\myimage{./illustrations/chap2/histogram_of_density/paris_vosges_density_histogramm.png}{ Histogram of number of points per patch, with a logarithmic scale for X and Y axis}{lod.fig:hist-density-dataset}
	...
	\subsection{MidOc: an ordering for gradual geometrical approximation}
		%\subsubsection{Experiment summary}
		\myimageFullPageWidth{./illustrations/chap2/LOD/visual_result_left_right}{Schematic illustration of different LOD. Left to right, all points, then LOD 4 to 0. Visualized in cloud compare with ambient occlusion. Point size varies for better visual result.}{lod.fig:visual_LOD_left_right}
	...
	\subsection{Excessive Density detection and correction} 
		\myimage{./illustrations/chap2/density/density_detection_and_correction}{Abnormal density detection and correction. Top points per patch (left) or density (right), green-yellow-red. Bottom reflectance in grey. }{lod.fig:density-correction}
	...
	\subsection{Rough Dimensionality descriptor}
		...
		\myimage{"./illustrations/chap2/comparing_dim_desc/success_case"}{$Dim_{LOD}$ and $Dim_{cov}$ are mostly comparable, except for few patches (5\%, coloured)}{lod.fig:success_case}
		...
		\myimage{"./illustrations/chap2/comparing_dim_desc/explaining_failure_case"}{Example of patches explaining most fo the differences between $Dim_{LOD}$ and $Dim_{cov}$ }{lod.fig:explaining_failure_case}
		...
		\myimage{./illustrations/chap2/classif/class_clustering/only_lod_feature_2.png}{Spectral clustering of confusion matrix of Paris data set classification using only $ppl$ descriptor. Edge width and colour are proportional to affinity. Node position is determined fully automatically. Red-ish arrows are manually placed to help understand}{lod.fig:ppl-separator-power}
	...
	\subsection{Patch Classification}
		...
		\myimage{./illustrations/chap2/classif/class_clustering/class_clustering_macro_all}{Result of automatic spectral clustering over confusion matrix for patch classification of Paris data set with all simple features. Edges width and colour are proportional to confusion. We manually draw clusters for easier understanding.}{lod.fig:class-clustering-all-features}
		...
		\myimage{./illustrations/chap2/classif/result_vosges/result_vosges_2.png}{Feature usage (first table).  Precision(prec.), recall (rec.), support (supp.), and average percent of points of the class in the patches, for comparison with point based method (mix.).}{lod.fig:result-vosges}
		...
		\myimageFullPageWidth{./illustrations/chap2/classif/result_paris/result_per_class_multilevel_paris_2.pdf}{Results for Paris data set: at various level of class hierarchy. Precision(prec.), recall (rec.), support (sup.) and average percent of points of the class in the patches of the class, for comparison with point based method (mix.). Classes of the same type are in the same continuous tone. Feature usage is evaluated for each level in the class hierarchy.}{lod.fig:result-paris} 
		...
		\myimage{./illustrations/chap2/precision_vs_recall/patch_classification_for_building.pdf}{Plotting of patches classified as building, using confidence to increase precision. Ground truth from IGN and Open Data Paris}{lod.fig:precision-increase}.
